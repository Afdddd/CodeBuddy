...
public class CompanyAdminController {
	...
	@GetMapping(value="adminListLoad.cp", produces="text/html; charset=UTF-8") @ResponseBody public String adminListLoad(String page) {
		...
		if(pi.getMaxPage() == 0) { return "none"; }
			...
			for (Company c: co) { if(c.getCompanyDelete() != null) c.setCompanyDelete("<button type='button' class='btn btn-info' onclick='onRestore(this, " + c.getCompanyNo() + ");'>복원시키기</button>"); else c.setCompanyDelete("<button type='button' class='btn btn-danger' onclick='onDelete(this, " + c.getCompanyNo() + ");'>탈퇴시키기</button>"); str_return += "<tr><td style='border-top: 1px solid #5271FF;'>" + c.getCompanyNo() + "</td><td style='border-top: 1px solid #5271FF;'>" + c.getCompanyName() + "</td><td style='border-top: 1px solid #5271FF;'>" + c.getCompanyOwner() + "</td><td style='border-top: 1px solid #5271FF;'>" + c.getCompanyBno() + "</td><td style='border-top: 1px solid #5271FF;'>" + c.getCompanyInfo() + "</td><td style='border-top: 1px solid #5271FF;'>" + c.getCompanyId() + "</td><td style='border-top: 1px solid #5271FF;'>" + c.getCompanyEmail() + "</td><td style='border-top: 1px solid #5271FF;'>" + c.getCompanyInsert() + "</td><td style='border-top: 1px solid #5271FF;'>" + c.getCompanyDelete() + "</td></tr>"; }
			return str_return;
		}
	}
